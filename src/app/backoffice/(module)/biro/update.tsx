...
import { createBiro, updateBiro } from "@/services/biro-service"
import { zodResolver } from "@hookform/resolvers/zod"
import { useState } from "react"
import { useForm } from "react-hook-form"

export const Update = ({ id, name }: { id: string, name: string }) => {
    ...
    const submitHandler = async (data: BiroDto) => {
        try {
            await updateBiro(id, data)
            toast({
                title: "Berhasil Ubah Biro",
                variant: "default",
            })
            form.reset()
            setOpen(false)
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen} >
            ...
            <DialogContent onKeyDown={(e) => handleKeyDown(e)
                ...
                <DialogHeader>
                    <DialogTitle>Ubah Biro</DialogTitle>
                </DialogHeader>
                <Form {...form}>
