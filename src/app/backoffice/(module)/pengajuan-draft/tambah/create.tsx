...
const useDraft = () => {
  const [draft, setDraft] = useState<DefaultValues<DraftCreateDto>>({
    title: "",
    content: "",
    publishedAt: null,
    category: "",
  ...
  const saveDraft = (data: DefaultValues<DraftCreateDto>) => {
    if (typeof window !== "undefined") {
...
type CategoryType = Prettify<
  PublicationCategory & { subs: PublicationSubCategory[] }
>;
...
const CreateFeature = () => {
  ...
  const submitHandler = async ({
    ...
    try {
      const formData = objectToFormdata({
        ...
        vote_descriptions: includeVote
          ? vote.map((item) => item.description)
          : null,
  ...
  return (
    <Form {...form}>
      <form
        onSubmit={form.handleSubmit(submitHandler, (err) => console.log(err))}
        className="space-y-3"
      >
        <FormField
          control={form.control}
        ...
        <LabelCheckBox
          id="thumbnail"
          label="Tambahkan Foto Sampul"
          checked={includeThumbnail}
          onCheckedChange={() => setIncludeThumbnail(!includeThumbnail)}
        />
        {includeThumbnail && (
          <div className=" border rounded p-3">
            <FormItem>
              <Input
                placeholder="Foto Sampul"
                type={"file"}
                onChange={(e) => {
        ...
        <LabelCheckBox
          id="source"
          label="Tambahkan Tautan Sumber"
          checked={includeLink}
          onCheckedChange={() => setIncludeLink(!includeLink)}
        />
        {includeLink && (
          <div className=" border rounded p-3">
            <FormField
        ...
        <LabelCheckBox
          id="video"
          label="Tambahkan Tautan Video"
          checked={includeVideo}
          onCheckedChange={() => setIncludeVideo(!includeVideo)}
        />
        {includeVideo && (
          <div className=" border rounded p-3">
            <FormField
        ...
        <LabelCheckBox
          ...
          checked={includeFile}
          onCheckedChange={() => setIncludeFile(!includeFile)}
        />
        {includeFile && (
          <div className="space-y-3 border p-3 rounded">
            <FormItem>
        ...
        <LabelCheckBox
          id="vote"
          label="Tambahkan Voting"
          checked={includeVote}
          onCheckedChange={() => setIncludeVote(!includeVote)}
        />
        {includeVote && (
          <div className="space-y-3 border p-3 rounded">
            <FormField
                        ...
                        <FormControl>
                          ...
                          >
                            {field.value ? (
                              ...
                              <span>Pilih Tanggal Terakhir Voting</span>
                            )}
                            <CalendarIcon className="ml-auto h-4 w-4 opacity-50" />
                          </Button>
                        </FormControl>
            ...
            {voteList.fields.map((item, i) => (
              <div className="space-y-3" key={i}>
                    ...
                    <FormItem className="">
                      <FormControl>
                        <TextareaAutosize
                          ...
                          className="p-3 w-full  focus:outline-none text-foreground border bg-transparent rounded"
                          {...field}
                        ></TextareaAutosize>
                      </FormControl>
                      <FormMessage />
                ...
                <Input
                  ...
                  type={"file"}
                  accept="image/png, image/gif, image/jpeg"
                  onChange={(e) => onFileVote(e.target.files, i)}
                />
              </div>
            ))}
