...
import { createDraftService, updateDraftService } from "@/services/draft-service"
import { findAllCategory } from "@/services/publication-service"
import { Prettify } from "@/types/utils"
import { zodResolver } from "@hookform/resolvers/zod"
import { PublicationCategory, PublicationSubCategory, Draft } from "@prisma/client"
...
const UpdateFeature = ({initialData}: UpdateFeatureProps) => {
    ...
    const form = useForm<DraftCreateDto>({
        ...
        defaultValues: {
            ...
            sub: initialData.subCategoryId ?? null,
            closedAt: null,
            linkSource: initialData.sourceLink ?? "",
            linkVideo: initialData.videoLink ?? "",
        }
    })
    ...
    const submitHandler = async ({ title, content, publishedAt, category, sub, linkSource, linkVideo, vote, closedAt }: DraftCreateDto) => {
        try {
            ...
            await updateDraftService(initialData.id, formData)
            toast({
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(submitHandler, (err) => console.log(err))} className="space-y-3">
                ...
                <Separator />
                <LabelCheckBox id="thumbnail" label="Ubah Foto Sampul" checked={includeThumbnail} onCheckedChange={() => setIncludeThumbnail(!includeThumbnail)} />
                {includeThumbnail && (
                ...
                <LabelCheckBox id="source" label="Ubah Tautan Sumber" checked={includeLink} onCheckedChange={() => setIncludeLink(!includeLink)} />
                {includeLink && (
                    <div className=" border rounded p-3">
                ...
                <LabelCheckBox id="video" label="Ubah Tautan Video" checked={includeVideo} onCheckedChange={() => setIncludeVideo(!includeVideo)} />
                {includeVideo && (
                    <div className=" border rounded p-3">
                ...
                <LabelCheckBox id="documents" label="Ubah Dokumen" checked={includeFile} onCheckedChange={() => setIncludeFile(!includeFile)} />
                {includeFile && (
                    <div className="space-y-3 border p-3 rounded">
                        <FormItem>
                ...
                <LabelCheckBox id="vote" label="Ubah Voting" checked={includeVote} onCheckedChange={() => setIncludeVote(!includeVote)} />
                {includeVote && (
                    <div className="space-y-3 border p-3 rounded">
                ...
                <div className=" w-full flex justify-end gap-3">
                    ...
                    <Button className={cn({ "bg-muted-foreground": form.formState.isSubmitting })}>{form.formState.isSubmitting ? "Mengunggah..." : "UBAH"}</Button>
                </div>
            </form>
        </Form>
    )
