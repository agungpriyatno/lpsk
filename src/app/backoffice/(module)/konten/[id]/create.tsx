...
import { PublicationCategory, PublicationSubCategory, Draft, DraftMedia, VoteDraft } from "@prisma/client"
...
const CreateFeature = ({ initial }: { initial: Props, }) => {
    ...
    const [published, setPublished] = useState(initial.publishedAt != null)
    const [includeLink, setIncludeLink] = useState(initial.sourceLink != null)
    const [includeVideo, setIncludeVideo] = useState(initial.videoLink != null)
    ...
    const form = useForm<DraftCreateDto>({
        ...
        defaultValues: {
            ...
            sub: initial.subCategoryId,
            linkSource: initial.sourceLink ?? undefined,
            linkVideo: initial.videoLink ?? undefined,
            closedAt: null,
        }
    })
    ...
    return (
        <div className=" space-y-3">
            <Button onClick={() => setOpen(!open)}>Ubah Konten</Button>
            {open && (
                    ...
                    <form onSubmit={form.handleSubmit(submitHandler, (err) => console.log(err))} className="space-y-3">
                        ...
                        <FormField control={form.control} name={`category`} render={({ field }) => (
                            <FormItem className="flex-1">
                                ...
                                <Select onValueChange={field.onChange} defaultValue={initial.categoryId ?? ""} onOpenChange={findOptions} >
                        ...
                        )} />
                        {
                            form.getValues("category") != null && <FormField control={form.control} name={`sub`} render={({ field }) => (
                                <FormItem className="flex-1">
                                    ...
                                    <Select onValueChange={field.onChange} defaultValue={initial.subCategoryId ?? ""} >
                                        <FormControl className="w-full">
                                            <SelectTrigger className="w-full">
