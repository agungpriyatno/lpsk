...
const Page = async ({ searchParams: { status, search, skip, take } }: PageProps) => {
    ...
    const list = await db.publication.findMany({
        ...
        where: { AND: [{ selected: { category: { code: "LPSK-PERATURAN" } } }, { status: "PUBLISH" }, search != undefined ? { selected: { title: { contains: search } } } : {}, status != undefined ? { selected: { subCategoryId: status } } : {}] }
    })

    const count = await db.publication.count({
        where: { AND: [{ selected: { category: { code: "LPSK-PERATURAN" } } }, { status: "PUBLISH" }, status != undefined ? { selected: { title: { contains: search } } } : {}, status != undefined ? { selected: { subCategoryId: status } } : {}] }
    })
    return (
                    ...
                    <div className="bg-background rounded relative overflow-hidden group" key={id}>
                        <AspectRatio ratio={1 / 1}>
                            ...
                            <div className="relative w-full h-full bg-slate-800">
                                <Image src={process.env.BUCKET_URL_ACCESS + (selected?.thumbnail ?? "default_zz.jpg")} alt="" className=" object-cover opacity-70 group-hover:scale-125 duration-300 transition-all" fill sizes="100vh" />
                            </div>
