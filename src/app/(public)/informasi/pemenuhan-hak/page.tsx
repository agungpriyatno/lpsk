...
export default async function Page() {
    ...
    const data = await db.draft.findMany({ where: { subCategoryId: "clrfsr1mu0009d7uyhcrdw1zu" }, include: { media: {include: {media: true}} } })
            ...
            <div className=' h-[400px] w-full bg-background'>
                <div className=' flex w-full h-full justify-center place-items-center relative'>
                    ...
                    <div className=' absolute left-0 top-0 right-0 bottom-0 flex justify-center place-items-center'>
                        <h1 className=' text-3xl font-bold absolute text-slate-100'>Informasi Pelayanan Pemenuhan Hak</h1>
                    </div>
                </div>
            </div>
