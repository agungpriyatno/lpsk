...
const Page = async ({ searchParams: { status, search, skip, take } }: PageProps) => {
    ...
    const list = await db.publication.findMany({
        ...
        where: { AND: [{ selected: { category: { code: "LPSK-BERITA" } } }, { status: "PUBLISH" }, search != undefined ? { selected: { title: { contains: search, mode: "insensitive" } } } : {}, status != undefined ? { selected: { subCategoryId: status } } : {}] }
    })

    const count = await db.publication.count({
        where: { AND: [{ selected: { category: { code: "LPSK-BERITA" } } }, { status: "PUBLISH" }, status != undefined ? { selected: { title: { contains: search, mode: "insensitive" } } } : {}, status != undefined ? { selected: { subCategoryId: status } } : {}] }
    })
    return (
