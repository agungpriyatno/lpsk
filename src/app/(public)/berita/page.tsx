...
import { fetchNews } from "@/services/fetching";
...
const Page = async ({
  searchParams: { status, search, skip, take },
}: PageProps) => {
  const data = await db.publicationCategory.findFirstOrThrow({
    where: { code: "LPSK-BERITA" },
    include: { subs: true },
  });
    ...
    <div className=" space-y-5 w-full pb-16">
      ...
      <AppContainer className=" grid grid-cols-1 md:grid-cols-4 2xl:grid-cols-6 gap-3">
        {list.map(({ selected, id }) => (
          <div
            className="bg-background rounded relative overflow-hidden group"
            key={id}
          >
            <AspectRatio ratio={1 / 1}>
              <Link href={"/berita/" + id}>
                <div className="flex flex-col p-5 absolute left-0 top-0 w-full h-full justify-end z-10 text-slate-50">
                  ...
                  <small className="text-xs">
                    Diunggah oleh {selected?.author?.name}
                  </small>
                </div>
              </Link>
              <div className="relative w-full h-full bg-slate-800">
                <Image
                  ...
                  alt=""
                  className=" object-cover opacity-70 group-hover:scale-125 duration-300 transition-all"
                  fill
                  sizes="100vh"
                />
              </div>
