...
export const findManyPublicationService = ({
    ...
    return db.publication.findMany({
      ...
      where: {
        AND: [
          ...
          {
            OR: [
              { selected: { title: { contains: search } } },
              { selected: { content: { contains: search } } },
            ],
          },
        ],
      },
...
export const findManyPublicationasdService = ({
  query: { search, skip, take },
  status,
}: FindManyPublication) => {
  if (status != undefined) {
    return db.publication.findMany({
      ...
      where: {
        AND: [
          { status },
          { OR: [{ selected: { title: { contains: search } } }] },
        ],
      },
...
export const findPublicationService = async (id: string) => {
  return await db.publication.findUniqueOrThrow({
    where: { id },
    include: {
      author: true,
      selected: {
          ...
          vote: {
            include: {
              vote: {
                include: {
                  options: {
                    include: { _count: { select: { client: true } } },
                  },
                },
              },
            },
...
export const createPubCategory = async ({ name, subs }: PubCategoryDto) => {
  await db.publicationCategory.update({
    where: { code: name },
    data: { subs: { createMany: { data: subs } } },
  });
  revalidatePath("/backoffice/konten/kategori");
};

export const findManyPubCategory = async ({
  query: { search, skip, take },
}: FindManyPubCategory) => {
...
export const deleteContentService = async ({ id }: { id: string }) => {
  const data = await db.publication.findFirstOrThrow({
    where: { id },
    include: { selected: true },
  });
  if (data.selectedId)
    await db.draft.delete({ where: { id: data.selectedId } });
  await db.publication.delete({ where: { id } });
  revalidatePath("/backoffice/konten");
};
